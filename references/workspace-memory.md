...
Keep Workspace writes as narrow as live-site writes. Use current revision/version identity for overwrite-sensitive updates when supported. If a Workspace write is stale/conflicted, re-read and reconcile newer valid state before retrying. If a write outcome is ambiguous, re-read first; never retry blindly in a way that can duplicate or erase newer context.
